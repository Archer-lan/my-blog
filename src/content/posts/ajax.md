...
title: AJAX 原理与简易封装
date: 2026-04-07
description: 解析 XMLHttpRequest 工作流程，从创建对象、建立连接、发送数据到 onreadystatechange 监听，并给出一个简易 ajax 函数封装。
category: JavaScript
tags: [JavaScript, AJAX, XMLHttpRequest, HTTP]
lang: zh
draft: false
---
...
> [原文有配图（readyState 状态表），但下载失败]

`readyState` 共有五个状态值，只要 `readyState` 属性值一变化，就会触发一次 `readystatechange` 事件。
